...
wu_inference_naive_baseline_dev:
	python3 ./naive_baseline/inference.py --gpu $(GPU) --cinnamon-data-path $(CINNAMON_DATA_PATH) --load-model ./naive_baseline/ckpt/naive/epoch_60.pt --ref-file /media/D/ADL2020-SPRING/project/cinnamon/dev/dev_ref.csv --dev_or_test dev --model naive --delta $(DELTA) --score --postprocess
wu_inference_naive_baseline_test:
	python3 ./naive_baseline/inference.py --gpu $(GPU) --cinnamon-data-path $(CINNAMON_DATA_PATH) --load-model ./naive_baseline/ckpt/naive/epoch_60.pt --dev_or_test test --model naive --delta $(DELTA) --postprocess
